...
use crate::{
    error::PushError,
    mmcsp::{self, authorize_put_response::UploadTarget, Container as ProtoContainer, HttpRequest},
    util::{make_reqwest, plist_to_bin},
    APNSConnection,
};
...
fn confirm_for_resp(
    ...
    mmcsp::confirm_response::Request {
        ...
        edge_info: [vec![mmcsp::confirm_response::request::Metric {
...
pub async fn prepare_put(reader: &mut dyn Container) -> Result<PreparedPut, PushError> {
    ...
    Ok(PreparedPut {
        total_sig: [vec![0x81], total_hasher.finish().to_vec()].concat(),
        chunk_sigs,
        total_len,
    })
}
    ...
    async fn ensure_stream(&mut self) {
        if self.sender.is_none() {
            ...
            let task = tokio::spawn(async move {
                let response =
                    transfer_mmcs_container(&make_reqwest(), &request, Some(body)).await?;
...
impl Container for MMCSPutContainer {
    ...
    async fn finalize(&mut self) -> Result<Option<mmcsp::confirm_response::Request>, PushError> {
        ...
        let result = self
            ...
            .as_ref()
            .unwrap()
            .send_async(Ok([(buf.len() as u32).to_be_bytes().to_vec(), buf].concat()))
            .await;
...
pub async fn put_mmcs(
    ...
    apns.send_message("com.apple.madrid", &binary, Some(&msg_id))
        .await?;

    let response = apns
        .reader
        .wait_find_msg(move |loaded| {
            ...
            c.as_unsigned_integer().unwrap() == 150
                && i.as_unsigned_integer().unwrap() as u32 == u32::from_be_bytes(msg_id)
        })
        .await;
    let apns_response: MMCSUploadResponse =
        plist::from_bytes(response.get_field(3).unwrap()).unwrap();

    let response =
        mmcsp::AuthorizePutResponse::decode(&mut Cursor::new(apns_response.response)).unwrap();
    let sources = vec![ChunkedContainer::new(prepared.chunk_sigs.clone(), source)];

    let mut put_containers: Vec<Box<MMCSPutContainer>> = response
        .targets
        .iter()
        .map(|target| {
            let len = target.chunks.iter().fold(0, |acc, chunk| {
                let wanted_chunk = prepared
                    .chunk_sigs
                    .iter()
                    .find(|test| &test.0[..] == &chunk.chunk_id[..])
                    .unwrap();
                wanted_chunk.1 + acc
            });
            Box::new(MMCSPutContainer::new(
                target.clone(),
                len,
        ...
        .collect();
    let targets = put_containers
        .iter_mut()
        .map(|target| ChunkedContainer::new(target.get_chunks(), target.as_mut()))
        .collect();
...
pub async fn transfer_mmcs_container(
    ...
    .header(
        "user-agent",
        "IMTransferAgent/1000 CFNetwork/1335.0.3.4 Darwin/21.6.0",
    );
...
impl MMCSMatcher<'_, '_> {
    ...
    fn best_source<'a, 'b, 'c>(
        targets: &Vec<ChunkedContainer<'b>>,
        sources: &'a mut Vec<ChunkedContainer<'c>>,
    ) -> Option<&'a mut ChunkedContainer<'c>> {
        let wanted = sources
            .iter()
            .enumerate()
            ...
            .max_by_key(|source| {
                targets
                    .iter()
                    .filter(|target| target.wanted_chunk() == Some(source.1.chunks[0].0))
                    .count()
            })
            .or_else(|| {
                sources
...
impl MMCSGetContainer {
    ...
    fn get_chunks(&self) -> Vec<([u8; 21], usize)> {
        self.container
            ...
            .map(|chunk| {
                (
                    chunk.meta.clone().unwrap().checksum.try_into().unwrap(),
                    chunk.meta.as_ref().unwrap().size as usize,
                )
            })
            .collect()
    }
    ...
    async fn ensure_stream(&mut self) {
        if self.response.is_none() {
            ...
            .await
            .unwrap();
            self.confirm = Some(confirm_for_resp(
                &response,
...
pub async fn get_mmcs(
    ...
    let total_bytes = response
        ...
        .fold(0, |acc, container| {
            acc + container
                .chunks
                .iter()
                .fold(0, |acc, chunk| acc + chunk.meta.as_ref().unwrap().size)
        }) as usize;

    let mut mmcs_sources: Vec<Box<MMCSGetContainer>> = response
        .f1
        .as_ref()
        .unwrap()
        .containers
        .iter()
        .map(|container| Box::new(MMCSGetContainer::new(container.clone())))
        .collect();
    let sources = mmcs_sources
        .iter_mut()
        .map(|container| ChunkedContainer::new(container.get_chunks(), container.as_mut()))
        .collect();

    let wanted_chunks = &response
        .f1
        .as_ref()
        .unwrap()
        .references
    ...
    let targets = vec![ChunkedContainer::new(
        wanted_chunks
            .iter()
            .map(|chunk| {
    ...
    let resp = send_mmcs_req(
        ...
        url,
        "getComplete",
        &format!("{} {}", data[0].cl_auth_p1, data[0].cl_auth_p2),
        &apns_response.object,
        &buf,
    )
    .await?;
