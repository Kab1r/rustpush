...
use crate::{
    apns::{APNSConnection, APNSState},
    bags::{get_bag, IDS_BAG},
    error::PushError,
    ids::signing::auth_sign_req,
    util::{gzip, make_reqwest, plist_to_bin, plist_to_string, ungzip, KeyPair},
};
use log::info;
use openssl::{
    bn::BigNum,
...
use super::{
    identity::{IDSIdentity, IDSPublicIdentity},
    signing::add_id_signature,
};
...
async fn attempt_auth(username: &str, password: &str) -> Result<Value, PushError> {
    ...
    let resp = client
        .post("https://profile.ess.apple.com/WebObjects/VCProfileService.woa/wa/authenticateUser")
...
async fn authenticate(
    ...
    let client = make_reqwest();
    let resp = client
        .post(ids_bag.get(endpoint_key).unwrap().as_string().unwrap())
    ...
    Ok(KeyPair {
        cert: cert,
        private: private_key.private_key_to_der()?,
    })
}
...
async fn get_phone_cert(
    phone_number: &str,
    push_token: &[u8],
    phone_signatures: &[Vec<u8>],
) -> Result<KeyPair, PushError> {
    authenticate(
        &format!("P:{}", phone_number),
        plist::to_value(&AuthPhoneNumber {
            push_token: push_token.to_vec().into(),
            sigs: phone_signatures
                .iter()
                .map(|number| number.clone().into())
                .collect(),
        })?,
...
pub async fn get_handles(
    ...
    let resp = auth_sign_req(
        client
            .get(ids_bag.get("id-get-handles").unwrap().as_string().unwrap())
...
impl IDSUser {
    ...
    pub async fn possible_handles(
        &self,
        conn: Arc<APNSConnection>,
    ) -> Result<Vec<String>, PushError> {
    ...
    pub async fn lookup(
        ...
        conn: Arc<APNSConnection>,
        query: Vec<String>,
    ) -> Result<HashMap<String, Vec<IDSIdentityResult>>, PushError> {
        ...
        add_id_signature(
            ...
            &encoded,
            "id-query",
            self.identity.as_ref().unwrap().id_keypair.as_ref().unwrap(),
            &conn.state.token.as_ref().unwrap(),
        )?;
        ...
        let request = Value::Dictionary(Dictionary::from_iter(
            [
                ...
                (
                    "u",
                    ids_bag.get("id-query").unwrap().as_string().unwrap().into(),
                ),
                ("h", headers.into()),
                ("v", 2.into()),
        ...
        conn.send_message("com.apple.madrid", &plist_to_bin(&request)?, None)
            .await?;
        ...
        let answer: HashMap<String, Vec<IDSIdentityResult>> =
            HashMap::from_iter(reps.iter().map(|(id, resp)| {
                (
                    id.clone(),
                    resp.identities
                        .iter()
                        .filter(|identity| identity.client_data.is_some())
                        .map(|identity| {
                            let key: Vec<u8> = identity
                                .client_data
                                .as_ref()
                                .unwrap()
...
impl IDSPhoneUser {
    pub async fn authenticate(
        ...
        let auth_keypair = get_phone_cert(
            phone_number,
            conn.state.token.as_ref().unwrap(),
            &[phone_sig.to_vec()],
        )
