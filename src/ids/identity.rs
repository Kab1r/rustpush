...
use crate::{
    apns::APNSConnection,
    error::PushError,
    util::{base64_decode, make_reqwest, plist_to_string, KeyPair},
};

use super::{
    signing::auth_sign_req,
    user::{IDSUser, IDSUserType},
};
...
impl IDSPublicIdentity {
    pub fn decode(data: &[u8]) -> Result<IDSPublicIdentity, PushError> {
        ...
        if &data[72..83]
            != &[
                0x82, 0x81, 0xAE, 0x00, 0xAC, 0x30, 0x81, 0xA9, 0x02, 0x81, 0xA1,
            ]
        {
            panic!("Bad ids!");
        }
...
fn encode_ec(ec: &EcPointRef) -> Vec<u8> {
    ...
    ec.affine_coordinates(&ec_group, &mut x, &mut y, &mut ctx)
        .unwrap();
    [
        vec![0x00, 0x41, 0x04],
...
impl IDSIdentity {
    pub fn new() -> Result<IDSIdentity, PushError> {
        let encryption_key = PKey::from_rsa(Rsa::generate_with_e(
            1280,
    ...
    pub fn public(&self) -> IDSPublicIdentity {
        ...
        let rsakey =
            Rsa::public_key_from_der(&encryption_key.rsa().unwrap().public_key_to_der().unwrap())
                .unwrap();
        IDSPublicIdentity {
            signing_key: public,
            encryption_key: rsakey,
        }
    }
...
pub async fn register(
    ...
    for user in users.iter_mut() {
        ...
        let identity = user
            .identity
            .get_or_insert_with(|| IDSIdentity::new().unwrap());
    ...
    let body = Value::Dictionary(Dictionary::from_iter(
        [
            (
                "hardware-version",
                Value::String("MacBookPro18,3".to_string()),
            ),
            ("language", Value::String("en-US".to_string())),
            (
                "os-version",
                Value::String("macOS,13.2.1,22D68".to_string()),
            ),
    ...
    let mut builder = client
        .get("https://identity.ess.apple.com/WebObjects/TDIdentityService.woa/wa/register")
        .header("x-protocol-version", "1640");
    for (idx, user) in users.iter().enumerate() {
        builder = auth_sign_req(
            builder.header(format!("x-auth-user-id-{}", idx), user.user_id.clone()),
            body.as_bytes(),
            "id-register",
            &user.auth_keypair,
            &conn.state,
    ...
    for user in users_array {
        ...
        user_obj.identity.as_mut().unwrap().id_keypair = Some(KeyPair {
            cert: cert.to_vec(),
            private: user_obj.auth_keypair.private.clone(),
        })
    }
